    ...
    tabulator.value = new Tabulator(tableRef.value, {
      ...
      columns: [
        ...
        // For HTML table
        {
          ...
          download: false,
          formatter(cell) {
            const {identity, correlative}: any = cell.getData();
            return `<div>
                <div class="font-medium whitespace-nowrap">V-${identity}</div>
                <div class="text-xs text-slate-500 whitespace-nowrap">${correlative}</div>
              </div>`;
          },
        },
        {
          ...
          download: false,
          formatter(cell) {
            const {firstname, lastname}: any = cell.getData();
            return `<div>
                <p class="font-medium whitespace-nowrap">${lastname}</p>
                <p class="text-xs text-slate-500 whitespace-nowrap">${firstname}</p>
              </div>`;
          },
        },
        {
          ...
          download: false,
          formatter(cell) {
            const {departament, charge}: any = cell.getData();
            return `<div>
                <p class="font-medium whitespace-nowrap">${departament.name}</p>
                <p class="text-xs text-slate-500 whitespace-nowrap">${charge.name}</p>
              </div>`;
          },
        },
        {
        ...
        },
        {
          ...
          formatter(cell) {
            const response: any = cell.getData();
            return `<div class="flex items-center lg:justify-center ${response.answerable ? "text-success" : "text-danger"
              }">
                <i data-lucide="${response.answerable ? "check-square" : "x-square"}" class="w-4 h-4 mr-2"></i> ${response.answerable ? "RESPONSABLE" : "NO RESPONSABLE"
              }
              </div>`;
          },
        },
...
<template>
  <div>
    <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
      <h2 class="mr-auto text-lg font-medium">Gestión de empleados</h2>
      ...
      <Dialog.Panel>
        <div class="p-5 text-center">
          ...
          <div class="mt-2 text-slate-500">
            ¿Desea eliminar este empleado? <br />
